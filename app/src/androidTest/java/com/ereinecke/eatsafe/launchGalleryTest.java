...
public class launchGalleryTest {
    ...
    @Test
    public void launchGalleryActivityTest() {
        ...
        // Build a result to return from the Gallery app
        Intent resultData = new Intent();
        resultData.putExtra("data", icon);
        Instrumentation.ActivityResult result = new Instrumentation.ActivityResult(Activity.RESULT_OK, resultData);

        // Stub out the Gallery activity. When an intent is sent to the Camera, this tells Espresso
        // to respond with the ActivityResult we just created
        intending(hasAction(Intent.ACTION_PICK)).respondWith(result);

        // Now that we have the stub in place, click on the button in our app that launches into the Camera
        onView(withId(R.id.action_gallery)).perform(click());
